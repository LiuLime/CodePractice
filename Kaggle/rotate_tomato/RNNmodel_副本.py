...
small_train = trainset.shuffle(seed=42).select(range(5000))
...
# small_test = testset.shuffle(seed=42).select(range(1000))
# small_test_tokz = small_test.map(tokz, batched=True)
# small_test_tokz.set_format('torch', device=device)
# i = torch.tensor(range(len(small_test_tokz['input_ids'])))
# small_test_tokz_iter = TensorDataset(i, small_test_tokz['input_ids'])
# test_input = DataLoader(small_test_tokz_iter, batch_size=batch_size)
...
labels=[]
with torch.no_grad():
    for i, input in test_input:
        # print(input)
        preds_logit = model_load(input)
        # print(preds_logit)
        label = torch.argmax(preds_logit, dim=-1).detach().numpy()  # .detach().numpy()的组合用于输出从PyTorch张量转换为NumPy数组
        labels.extend(label)
    print(labels)
